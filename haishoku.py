...
class Haishoku(object):
    ...
    def getColorsMean(image_path):
        ...
        # return the most 10 colors
        temp_sorted_colors_mean = sorted(colors_mean)
        if 10 < len(temp_sorted_colors_mean):
            colors_mean = temp_sorted_colors_mean[len(temp_sorted_colors_mean)-10 : len(temp_sorted_colors_mean)]
        else:
    ...
    def showPalette(image_path):
        ...
        theP = haillow.joint_image(images)
        return theP

    def showPalette2(image_path):
        # get the palette first
        palette = Haishoku.getPalette2(image_path)
    ...
    def getPalette2(image_path=None):
        # get the colors_mean
        colors_mean = Haishoku.getColorsMean(image_path)
        ...
        palette = []
        for p in palette_tmp:
            if not(p[1][0] >= 200 and p[1][1] >= 200 and p[1][2] >= 200):
                pp = '%.2f' % (p[0] / count_sum)
                tp = (float(pp), p[1])
                palette.append(tp)
